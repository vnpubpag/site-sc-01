...
(async ()=>{
    ...
    function N() {
        return {
            dropzone: u("eip-dropzone"),
            fileInput: u("eip-file-input"),
            selectBtn: u("eip-select-btn"),
            workspace: u("eip-workspace"),
            filename: u("eip-filename"),
            filesize: u("eip-filesize"),
            removeBtn: u("eip-remove"),
            scanSection: u("eip-scan"),
            scanStatus: u("eip-scan-status"),
            readBtn: u("eip-read-btn"),
            summary: u("eip-summary"),
            pagesCount: u("eip-pages-count"),
            imagesCount: u("eip-images-count"),
            pagesSection: u("eip-pages-section"),
            pagesList: u("eip-pages-list"),
            extractedCounter: u("eip-extracted-counter"),
            pageCardTpl: u("eip-page-card-tpl"),
            actions: u("eip-actions"),
            extractAllBtn: u("eip-extract-all"),
            downloadZipBtn: u("eip-download-zip"),
            empty: u("eip-empty"),
            renderFallbackBtn: u("eip-render-fallback"),
            progress: u("eip-progress"),
            progressStatus: u("eip-progress-status"),
            progressPct: u("eip-progress-pct"),
            progressFill: u("eip-progress-fill")
        };
    }
    function U(t) {
    ...
    function G(t, e) {
        t.dropzone.style.display = "none", t.workspace.style.display = "", t.filename.textContent = e.name, t.filesize.textContent = U(e.size), t.summary.style.display = "none", t.pagesSection.style.display = "none", t.actions.style.display = "none", t.empty.style.display = "none", t.progress.style.display = "none", t.scanStatus.textContent = "Sẵn sàng đọc file", t.readBtn.disabled = !1;
    }
    function H(t) {
    ...
    function q(t, e, n) {
        ...
        t.imagesCount.textContent = String(a), V(t, e, n), t.pagesSection.style.display = "", t.actions.style.display = "", t.extractAllBtn.disabled = !1, t.downloadZipBtn.disabled = !0, b(t, 0), a === 0 ? t.empty.style.display = "" : t.empty.style.display = "none";
    }
    function V(t, e, n) {
    ...
    function J(t, e) {
        ...
        return a.textContent = String(e.pageIndex + 1), r.textContent = Z(e.imageCount, "ảnh"), i.textContent = e.hasRawImageHint ? "Có thể có ảnh gốc" : "Hình ảnh hiển thị", i.classList.toggle("badge--info", e.hasRawImageHint), i.classList.toggle("badge--default", !e.hasRawImageHint), o.textContent = "Sẵn sàng", {
            root: n,
            count: r,
            hint: i,
            status: o,
    ...
    const k = 200 * 1024 * 1024, E = 4e7, Y = 2;
    let v = null;
    function Q() {
        return v || (v = (async ()=>{
            const t = await X(()=>import("./pdf.C_7zF4kU.js").then(async (m)=>{
                    await m.__tla;
                    return m;
                }), []);
    ...
    async function et(t) {
        const { buffer: e, size: n } = await tt(t);
        if (n > k) throw new Error(`PDF quá lớn (${Math.round(n / 1024 / 1024)}MB). Tối đa ${Math.round(k / 1024 / 1024)}MB.`);
        const a = await Q();
        let r;
        try {
    ...
    async function T(t, e) {
        ...
        try {
            ...
            for(let l = 0; l < r.fnArray.length; l++){
                ...
                if (n.inlineSingle !== void 0 && d === n.inlineSingle) {
                    i++;
                    continue;
                }
                if (n.inlineGroup !== void 0 && d === n.inlineGroup) {
    ...
    async function ot(t, e, n) {
        ...
        for(let i = 0; i < a; i++){
            ...
            r[i] = await T(t, i), e?.(i + 1, a), await new Promise((o)=>setTimeout(o, 0));
        }
        return r;
    }
    function S(t, e) {
    ...
    async function M(t, e = "image/png", n) {
    ...
    async function gt(t, e) {
        ...
        const r = S(Math.ceil(a.width), Math.ceil(a.height));
        if (!r) return;
    ...
    async function pt(t, e, n) {
        ...
        const i = Math.ceil(r.width), o = Math.ceil(r.height), s = S(i, o);
        if (!s) return null;
        const { canvas: l, ctx: d } = s;
        try {
            await t.render({
                canvasContext: d,
                viewport: r
    ...
    async function ft(t, e, n = {}) {
        const { fallbackToRender: a = !0, renderScale: r = Y, signal: i } = n;
        if (i?.aborted) throw new DOMException("Aborted", "AbortError");
        const { pdfjsLib: o } = t, s = o.OPS, l = await t.getPage(e), d = [], c = new Set;
        try {
                ...
                for(let h = 0; h < g.fnArray.length; h++){
                    ...
                    try {
                        if (y === s.paintImageXObject || y === s.paintImageXObjectRepeat || y === s.paintJpegXObject) {
                            ...
                            c.add(p);
                            const I = await dt(l, p), w = await A(I);
                            w && d.push(L(e, d.length, w, "raw", p));
                            continue;
                        }
                        if (y === s.paintInlineImageXObject) {
                            const p = await A(f ? f[0] : null);
                            p && d.push(L(e, d.length, p, "raw"));
                            continue;
                        }
    ...
    class yt {
        ...
        async analyze(e, n) {
            ...
            try {
                const a = await ot(this.parsed, (r)=>{
                    this.transition({
                        processedPages: r,
        ...
        async analyzeOne(e) {
            return this.analysis?.[e] ? this.analysis[e] : T(this.parsed, e);
        }
        async extractPage(e, n = {}) {
            ...
            const r = (async ()=>{
                try {
                    const i = await ft(this.parsed, e, n);
    ...
    function wt(t, e = {}) {
        ...
        return t.map((s)=>{
            ...
            const d = i.get(s.pageIndex) ?? 1, c = Math.max(2, String(d).length), g = z(s.pageIndex + 1, r), x = z(l + 1, c);
            return {
    ...
    const Pt = 200 * 1024 * 1024, Ct = "Dữ liệu xử lý sẽ bị mất. Bạn có chắc muốn xóa file này?";
    async function _(t, e, n) {
        ...
        await D(e), e.file = n, e.phase = "idle", G(t, n);
    }
    async function Et(t, e) {
        if (!(!e.file || e.phase === "reading")) {
            e.phase = "reading", H(t);
            try {
                const n = await et(e.file);
                e.parsed = n;
                const a = new yt(n);
                e.controller = a;
                const r = await a.analyze();
                e.analysis = r, e.phase = "ready", q(t, e, r);
            } catch (n) {
                e.phase = "idle";
    ...
    async function St(t, e, n, a = {}) {
        ...
        try {
            ...
            R(r, i), r && (r.count.textContent = `${i.length} ảnh`), b(t, e.controller.getCache().size());
        } catch (i) {
            console.error("[extract-images] page extract failed:", i), W(r);
        }
    }
    async function $(t, e, n = {}) {
        if (!e.controller || !e.parsed || e.phase === "extracting") return;
        const a = new AbortController;
        e.abort = a, e.phase = "extracting", t.extractAllBtn.disabled = !0, t.empty.style.display = "none", C(t, !0), B(t, 0, e.parsed.numPages, "Đang chuẩn bị...");
        ...
        try {
            await e.controller.extractAll({
                ...
                signal: a.signal,
                onProgress: (s)=>{
                    typeof s.currentPageIndex == "number" && O(e.cards.get(s.currentPageIndex)), B(t, s.processedPages, s.totalPages, `Đã xử lý ${s.processedPages}/${s.totalPages} trang`), b(t, s.totalImages);
                }
            });
            ...
            for (const [s, l] of e.cards){
                const d = i.get(s) ?? [];
                d.length > 0 && (o = !0), R(l, d), l.count.textContent = `${d.length} ảnh`;
            }
            ...
            C(t, !1), e.phase = "ready", t.extractAllBtn.disabled = !1, e.abort = null;
        }
    }
    ...
    async function vt(t, e) {
        ...
        if (n.length !== 0) {
            t.downloadZipBtn.disabled = !0, C(t, !0), B(t, 0, 1, "Đang tạo file ZIP...");
            try {
                const a = await bt(n, {
                    rootFolder: "images",
                    compressionLevel: 6,
                ...
                Bt(a, Mt(e.file?.name ?? "pdf"));
            } catch (a) {
                console.error("[extract-images] zip build failed:", a), alert("Không tạo được file ZIP. Vui lòng thử lại.");
            } finally{
                C(t, !1), t.downloadZipBtn.disabled = !1;
            }
        }
    }
    async function At(t, e) {
        Lt(e) && !confirm(Ct) || (await D(e), K(t));
    }
    function Lt(t) {
        return !!(t.phase === "extracting" || t.analysis !== null || t.controller && t.controller.getCache().size() > 0);
    }
    async function D(t) {
        t.abort && (t.abort.abort(), t.abort = null), t.parsed && await nt(t.parsed), t.parsed = null, t.controller = null, t.analysis = null, t.cards.clear(), t.file = null, t.phase = "idle";
    }
    function Bt(t, e) {
    ...
    function F() {
        const t = N(), e = kt();
        t.selectBtn.addEventListener("click", (n)=>{
            ...
            n.preventDefault(), t.dropzone.classList.add("upload-zone--dragover");
        }), t.dropzone.addEventListener("dragleave", ()=>t.dropzone.classList.remove("upload-zone--dragover")), t.dropzone.addEventListener("drop", (n)=>{
            n.preventDefault(), t.dropzone.classList.remove("upload-zone--dragover");
