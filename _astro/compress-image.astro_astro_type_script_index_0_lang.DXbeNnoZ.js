...
(async ()=>{
    ...
    const g = [];
    let I = "reduce-size";
    const z = new Worker(new URL("/_astro/worker-_08ERztc.js", import.meta.url), {
        type: "module"
    }), d = document.getElementById("cimg-dropzone"), m = document.getElementById("cimg-file-input"), j = document.getElementById("cimg-select-btn"), J = document.getElementById("cimg-files-section"), K = document.getElementById("cimg-file-count"), E = document.getElementById("cimg-file-list"), V = document.getElementById("cimg-add-more"), M = document.getElementById("cimg-compress-all"), T = document.getElementById("cimg-download-all"), W = document.getElementById("cimg-file-row-tpl"), v = document.querySelectorAll(".cimg-mode__tab"), U = document.getElementById("cimg-panel-reduce"), x = document.getElementById("cimg-panel-convert");
    document.getElementById("cimg-mode");
    const F = document.getElementById("cimg-options"), O = document.getElementById("cimg-stats"), H = document.getElementById("cimg-stats-count"), Q = document.getElementById("cimg-stats-original"), Y = document.getElementById("cimg-stats-compressed"), ee = document.getElementById("cimg-stats-reduction");
    function te() {
    ...
    function _() {
        const e = c.size;
        J.style.display = e > 0 ? "" : "none", d.style.display = e > 0 ? "none" : "", K.textContent = e + " ảnh";
        const t = [
            ...c.values()
    ...
    function ne() {
        ...
        H.textContent = String(e.length), Q.textContent = p(t), Y.textContent = p(n), ee.textContent = s + "%";
    }
    function se(e) {
        const n = W.content.cloneNode(!0).querySelector(".cimg-file");
        n.dataset.id = e.id;
    ...
    function ie(e, t) {
        ...
        L.disabled = !1, L.querySelector(".cimg-file__dl-label").textContent = `Tải ${S}`, L.addEventListener("click", ()=>{
            ...
            const q = n.file.name.replace(/\.\w+$/, ""), X = $[n.bestFormat] || n.bestFormat;
            f.download = `${q}-compressed.${X}`, f.click(), URL.revokeObjectURL(f.href);
        });
    ...
    V.addEventListener("click", ()=>m.click());
